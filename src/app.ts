...
const loadComments = () => {
  for (let i = 0; i < data.comments.length; i++) {
    document.querySelector(
      `#comment${i + 1} .user`
    ).innerHTML = `<img src="${data.comments[i].user.image.png}"><div class="name">${data.comments[i].user.username}</div><div class="date">${data.comments[i].createdAt}</div>`;
    document.querySelector(
      `#comment${i + 1} .content`
    ).innerHTML = `<p>${data.comments[i].content}</p>`;
    document.querySelector(
      `#comment${i + 1} .score`
    ).innerHTML = `${data.comments[i].score}`;
    document.querySelector(
      `#reply${i + 1} .user`
    ).innerHTML = `<img src="${data.comments[1].replies[i].user.image.png}"><div class="name">${data.comments[1].replies[i].user.username}</div><div class="date">${data.comments[1].replies[i].createdAt}</div>`;
    document.querySelector(
      `#reply${1} .content`
    ).innerHTML = `<p><span class="mention">@maxblagun </span>${data.comments[1].replies[0].content}</p>`;
    document.querySelector(
      `#reply${2} .content`
    ).innerHTML = `<p><span class="mention">@ramsesmiron </span>${data.comments[1].replies[1].content}</p>`;
    document.querySelector(
      `#reply${i + 1} .score`
    ).innerHTML = `${data.comments[1].replies[i].score}`;
    document.querySelector(
      `#reply${2} .user`
    ).innerHTML = `<img src="${data.comments[1].replies[i].user.image.png}"><div class="name">${data.comments[1].replies[i].user.username}</div><div class="you">you</div><div class="date">${data.comments[1].replies[i].createdAt}</div>`;
  }
};
...
minus.forEach((item) => {
  item.addEventListener("click", function subtract() {
    if (
      score[minus.indexOf(item)].innerHTML >= startScore[minus.indexOf(item)]
    ) {
...
const textArea = document.getElementById("textMain") as HTMLTextAreaElement;
const createComment = () => {
...
const editing = (edit: NodeListOf<Node>) => {
  edit.forEach((button: HTMLButtonElement) => {
    if (
      button.classList.contains("newEditBug") ||
      ...
      button.addEventListener("click", (editing) => {
        ...
        newButton.addEventListener("click", (update) => {
          ...
          if (textEdited.value.split("")[0] == "@") {
            ...
            parentElement.children[1].innerHTML = `<p><span class="mention">${replyTo} </span>${textArea.join(
              " "
            )}</p>`;
          } else {
            parentElement.children[1].innerHTML = `<p>${textArea.join(
              " "
            )}</p>`;
          }
          newButton.remove();
...
const loadReply = () => {
  const textReply = document.querySelector(
    ".reply .textReply"
...
<section class="main">
  ...
  <div class="content"><span class="mention">${replyTo} </span>${textArea.join(
    " "
  )}</div>
</section>`;
...
replyButton.forEach((button) => {
  button.addEventListener("click", (reply) => {
    ...
    <textarea class="textReply">@${
      button.closest(".header").children[0].children[1].innerHTML
    }</textarea>
...
replyToReplyButton.addEventListener("click", (replyToFunc) => {
  ...
  newElement.innerHTML = `
    <img class="avatarComment" src="./images/avatars/image-juliusomo.png" />
    <textarea class="textReply">@${
      replyToReplyButton.closest(".header").children[0].children[1].innerHTML
    }</textarea>
    <button class="replySubmit">REPLY</button>`;
  replySection.appendChild(newElement);
  const replySubmit = document.querySelector(".replySubmit");
